...
import com.ssafy.ssafit.model.service.S3Service;
...
//				user.setPassword("");
	...
	public ResponseEntity<?> insertUser(User user, MultipartFile files) {
		try {
			if(files != null && files.getSize()>0) {
				String imgPath = s3Service.upload(files);
				user.setImage("https://" + "d38sfifral62ij.cloudfront.net" + "/" + imgPath);				
			}
	...
	public ResponseEntity<?> modifyUser(User user, MultipartFile files) throws IOException {
		if(files != null && files.getSize()>0) {
			String imgPath = s3Service.upload(files);
			user.setImage("https://" + "d38sfifral62ij.cloudfront.net" + "/" + imgPath);				
		}
